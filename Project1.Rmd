...
Last we average each player's opponents' average scores across all rounds that were played. The easiest way to do this appears to be using aggregation in a data table.
...
Finally, with the averages of each player's opponents availabe, we merge that data into the players data frame as the last column.
...
All that is left to do is export the results to a .csv file. Since we do not need to export the PairNum column we will set it to null.
